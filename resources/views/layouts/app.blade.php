        ...
        @auth
            ...
            <div class="lg:hidden border-b border-border bg-white px-4 py-2 overflow-x-auto flex gap-2 text-sm shrink-0">
                @if ($role === 'admin')
                    ...
                    <a href="{{ route('admin.semesters.index') }}" class="whitespace-nowrap px-3 py-1.5 rounded-lg {{ request()->routeIs('admin.semesters.*') ? 'bg-brand-50 text-brand-700 font-semibold' : 'text-slate-600' }}">Семестры</a>
                @elseif ($role === 'teacher')
